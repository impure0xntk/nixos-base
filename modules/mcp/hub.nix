...
in
{
  options.my.system.mcp.hub = {
    ...
    user = lib.mkOption {
      type = types.str;
      default = "mcpjungle";
      description = "User account under which mcpjungle run";
    };
    group = lib.mkOption {
      type = types.str;
      default = "mcpjungle";
      description = "Group under which mcpjungle run";
    };
  ...
  config = lib.mkIf config.my.system.mcp.hub.enable {
    ...
    # Create mcpjungle user and group
      ...
      postStart = let
        ...
        script = pkgs.writeShellApplication {
          ...
          text = ''
            ...
            for server in ${lib.concatStringsSep " " serverFiles}; do
              server_name="$(echo $server | cut -d, -f1)"
              server_file="$(echo $server | cut -d, -f2)"
